...
Qt3DCore::QBackendNode *PhysicsBufferFunctor::create(const Qt3DCore::QNodeCreatedChangeBasePtr &change) const
{
    if(m_manager->m_resources.contains(change->subjectId())){
        return m_manager->m_resources[change->subjectId()];
    }
    else{
        PhysicsBuffer* buffer=new PhysicsBuffer();
        m_manager->m_resources.insert(change->subjectId(),buffer);
